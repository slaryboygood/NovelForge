...
| `v4-11-agent-mode` | `agent` | `src/novelforge/agent/**`、`src/novelforge/application/services/agent.py`、`src/novelforge/api/agent_routes.py`、`persistence/paths.py`（agent_* 路径）、`ui/src/api/agent.ts`、`ui/src/studio/workspaces/Agent.tsx`、`tests/agent/**`、`tests/browser_v4_11_agent.cjs`、`docs/v4/**` | Agent 直接依赖 application / repository / store / provider / api / mcp、绕过 revision / approval / budget、自动接受或自动交付、业务模块反向 import agent、release tag、`story_engine/repair.py`（M11 frozen） | `AgentGoal` / `AgentPolicy` / `AgentPlan` / `AgentStep` / `AgentResult` / `AgentService` / Agent Ports / `AgentExecutor`（新增） | `tests/agent/**`、`tests/v4/isolation/test_agent_boundaries.py`、`ui/src/studio/**/*.test.tsx`、`tests/browser_v4_11_agent.cjs` |
