...
### 2.2 V4-10 主产品面迁移（不删除任何后端）

```text
Story Studio primary migration = DONE
  /                     → Story Studio（默认产品面，ADR-032）
  ?ui=v3 / #/v3...      → V3 工作台 = explicit compatibility
  ?ui=v2 / #/story-builder... → V2 面板 = explicit compatibility

Legacy entry = explicit compatibility
  8 个 tests/browser_v3_*.cjs 与 4 个 V2/creator 脚本只改启动参数（断言未改）；
  可执行门禁 tests/browser_v4_legacy_entry.cjs = PASS。

Legacy backend removal = NOT YET ELIGIBLE
  V3 工作台 / V2 creator 面板 / legacy writer / outline revision /
  legacy export backend endpoints 仍有真实兼容消费者（V2/V3 界面与历史验收脚本），
  V4-10 不删除它们；下表逐条保留移除条件。

Historical V3/V2 acceptance suites = NOT RUN（本环境缺少作者 acceptance data root）
  没有制造假数据、没有降低门槛、没有宣称 PASS。
```
