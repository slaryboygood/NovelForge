...
class StubProvider:
    """按 **task（LLMRequest.operation）** 返回结构化 JSON 的本地 provider。

    零网络、零真实模型；按 task 取值 → 与调用顺序无关（重复运行同一服务也稳定）。
    """
    ...
    def complete(self, request: Any) -> Any:
        ...
        item = self.script.get(str(getattr(request, "operation", "")), {})
...
def build_gateway(script: Mapping[str, Any]) -> Any:
...
#: 生成脚本（key = generation task；与调用顺序无关，可重复生成）
def generation_script() -> dict[str, Any]:
    return {
        "premise": {"premise": "一座断电边城必须在七十二小时内恢复供电",
        ...
        "world": {"rules": ["供电中断超过七十二小时城市将不可居住"],
        ...
        "character": {"name": "林澈", "role": "主角", "goal": "恢复供电",
