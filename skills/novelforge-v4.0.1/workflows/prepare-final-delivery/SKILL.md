---
name: novelforge-v4.0.1.workflows.prepare-final-delivery
description: 端到端流程：交付前检查（accepted + 质量）→ 预检 → 建立交付 → 校验 manifest → 下载。
---

# prepare-final-delivery

- **Skill ID**: `novelforge-v4.0.1.workflows.prepare-final-delivery`
...
Step 5 novelforge-v4.0.1.delivery.validate-delivery        → preflight（0 落盘）
...
```text
· Step 5 preflight ok=true；不 ok 时先回到 Step 1–4（不要放宽 policy 了事）
...
| 现象 | 原因 | 处理 |
| --- | --- | --- |
| `DELIVERY_VALIDATION_FAILED` | 未 accepted / 质量 blocker | 回到 Step 1–4 |
...
```text
skills/novelforge-v4.0.1/{delivery,editor,quality}/*
